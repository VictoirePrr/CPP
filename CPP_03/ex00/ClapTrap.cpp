...
/*   Updated: 2025/08/22 14:50:29 by victoire         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(const ClapTrap& other){
    ...
    std::cout << "ClapTrap copy constructor copied "<< _name << std::endl;
}
...
void ClapTrap::attack(const std::string& target) {
    if (_energyPoints <= 0 || _hitPoints <= 0)
        std::cout << "ClapTrap " << _name << " doesn't have any points left to attack!" 
        << std::endl;
    else 
    {
        _energyPoints--;
...
void ClapTrap::takeDamage(unsigned int amount){
    ...
    if (_energyPoints <= 0 || _hitPoints <= 0)
        ...
        << " damage and has now " << _hitPoints << " points of health! " << std::endl;
}

void ClapTrap::beRepaired(unsigned int amount) {
    if (_energyPoints <= 0 || _hitPoints <= 0)
        std::cout << "ClapTrap " << _name << " doesn't have any points left to get repaired!" 
        << std::endl;
    else
    { 
        _hitPoints += amount;
