...
void ClapTrap::attack(const std::string& target) {
    if (_energyPoints <= 0 || _hitPoints <= 0)
        std::cout << "ClapTrap " << _name << " doesn´t have any points left to attack!" << std::endl;
    else 
    {
        _energyPoints--;
...
void ClapTrap::beRepaired(unsigned int amount) {
    if (_energyPoints <= 0 || _hitPoints <= 0)
        std::cout << "ClapTrap " << _name << " doesn´t have any points left to get repaired!" << std::endl;
    else
    { 
        _hitPoints += amount;
        _energyPoints--;
        std::cout << "ClapTrap " << _name << " repaired itself with " << amount <<" points, restoring its health to " << _hitPoints << std::endl;
    }
